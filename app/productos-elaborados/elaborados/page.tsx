...
export default function Elaborados() {
    ...
    async function fetchProductos() {
      try {
        const res = await fetch('/api/productos', {
          ...
          body: JSON.stringify({ categoria_id: 2, subcategoria_id: 6 }),
        })
      ...
      <div className={styles.pageContainer}>
        <aside className={`${styles.sidebar} flex flex-col`}>
          ...
          <ul className="space-y-2 pl-4 text-left">
            {['CHARCUTERIA'].map(cat => (
              <li key={cat}>
                <Link href={`/${cat.toLowerCase().replace(/ /g, '')}`} className="text-sm text-white hover:text-[#990000] transition-colors">
                  {cat}
                </Link>
              </li>
            ))}
          ...
          <div className="w-full bg-[rgb(22,22,22)] py-3 mb-4">
            <div className="max-w-screen-xl mx-auto px-4 flex items-center justify-between text-sm">
              <div className="flex items-center space-x-3">
                <Image src="/imagenes/iconos/aplicaciones.png" alt="Ver en rejilla" width={20} height={20} className={`cursor-pointer ${viewMode === 'grid' ? 'opacity-100' : 'opacity-50'}`} onClick={() => setViewMode('grid')} />
                <Image src="/imagenes/iconos/lista.png" alt="Ver en lista" width={20} height={20} className={`cursor-pointer ${viewMode === 'list' ? 'opacity-100' : 'opacity-50'}`} onClick={() => setViewMode('list')} />
                <span className="ml-3 text-white border-l border-gray-500 pl-3">Mostrando 1–{sortedProductos.length} de {sortedProductos.length}</span>
              </div>
  ...
  <p className="font-bold text-[#990000] text-left text-sm">{p.precio.toFixed(2)}€</p>
                  ...
                  <button onClick={() => handleAñadir(p)} disabled={!p.stock} className={`mt-auto w-full rounded text-sm px-3 py-2 transition ${p.stock ? 'bg-gray-200 text-gray-800 hover:bg-[#990000] hover:text-white' : 'bg-gray-400 text-gray-600 cursor-not-allowed'}`}>Añadir a la cesta</button>
                </div>
              ))}
            </div>
          ) : (
            <div className="flex flex-col space-y-4">
              {sortedProductos.map(p => (
                <div key={p.id} className={`${styles.productCardList} flex items-center justify-between p-4 bg-[rgba(0,0,0,0.8)] rounded-lg shadow-md transition-transform hover:-translate-y-1`}>
                  <Link href={`/detalle-productos/${p.id}`}>
                    <Image src={p.imagen.startsWith('http') ? p.imagen : `/imagenes/productos/${p.imagen}`} alt={p.nombre} width={260} height={240} className="rounded object-cover" unoptimized={p.imagen.startsWith('http')} />
                  </Link>
                  ...
                  <div className="product-info flex flex-col space-y-5 border-l border-gray-700 pl-4">
                    ...
                    <button onClick={() => handleAñadir(p)} disabled={!p.stock} className={`w-full rounded text-sm px-3 py-2 transition ${p.stock ? 'bg-gray-200 text-gray-800 hover:bg-[#990000] hover:text-white' : 'bg-gray-400 text-gray-600 cursor-not-allowed'}`}>Añadir a la cesta</button>
                  </div>
                </div>
              ))}
            </div>
